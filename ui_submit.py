...
def submit_action(to_email, cc_email, subject, body, attachments, selected_days, scheduled_time, selected_tables):
    ...
    # Optionally, you could also send the email immediately
    # email_flow_executor.send_email_via_outlook(email_details)

def save_flow(email_details, flow_name):
    flow_folder = 'email_flows'
    os.makedirs(flow_folder, exist_ok=True)
    flow_path = os.path.join(flow_folder, flow_name + '.json')
